          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1 md:gap-2">
            {tools.map((tool, i) => {
              ...
              const wrapperProps = tool.href
                ...
                : {};
              return (
                <motion.div key={tool.label} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  <Wrapper
                    {...wrapperProps}
                    className="flex flex-col items-center gap-3 p-5 md:p-6 rounded-xl hover:bg-primary hover:text-primary-foreground transition-all duration-300 group cursor-pointer"
                  >
                    <tool.icon size={28} className="text-primary group-hover:text-primary-foreground transition-colors" />
                    <span className="text-sm font-semibold font-body text-foreground group-hover:text-primary-foreground transition-colors">
                      {tool.label}
                    </span>
                  </Wrapper>
                </motion.div>
